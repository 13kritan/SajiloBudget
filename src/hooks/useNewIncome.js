...
export function useEsewaIncome() {
  ...
  const addIncome = async ({ amount, note, source }) => {
    ...
    try {
      ...
      await axios.post("https://sajilobudget-backend.onrender.com/api/income", {
...
export function useIncomes() {
  ...
  const fetchIncomes = async () => {
    ...
    try {
      const res = await axios.get("https://sajilobudget-backend.onrender.com/api/income", authConfig());
      setIncomes(res.data);
    } catch (err) {
      console.error(err);
